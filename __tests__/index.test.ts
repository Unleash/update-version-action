...
describe('action', () => {
    ...
    it(`fails if distribution is not 'oss' or 'enterprise'`, async () => {
        ...
        expect(setFailedMock).toHaveBeenCalledWith('Distribution must be either oss or enterprise');
    });
    ...
    it(`fails if version is not a valid semver version or a valid semver version prefixed with a single 'v'`, async () => {
        ...
        await index.run();
        expect(setFailedMock).toHaveBeenCalledWith('Version must be a valid semver version');
    });
